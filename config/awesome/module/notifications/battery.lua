...
local battery_threshold_critical = 5
...
awesome.connect_signal("evil::battery", function(battery)
    ...
    if not charger_plugged then
        ...
        if battery <= battery_threshold_critical and not battery_critical_already_notified then
            battery_critical_already_notified = true
            message = "CRITICAL"
            -- message = helpers.colorize_text("CRITICAL", x.color9)
            timeout = 0
        elseif battery <= battery_threshold_low and not battery_low_already_notified then
