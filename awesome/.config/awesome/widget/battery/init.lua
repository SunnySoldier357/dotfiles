...
local widget_button = clickableContainer(wibox.container.margin(widget, dpi(14),
    dpi(14), 4, 4))
...
-- Alternative to naughty.notify - tooltip. You can compare both and choose
--     the preferred one
local battery_popup =
    awful.tooltip(
    {
...
watch(
    ...
    function(_, stdout)
        ...
        for s in stdout:gmatch("[^\r\n]+") do
            local status, charge_str, time = string.match(s,
                ".+: (%a+), (%d?%d?%d)%%,?.*")
            if status ~= nil then
        ...
        if (charge >= 0 and charge < 15) then
            if status ~= "Charging" and os.difftime(os.time(),
                last_battery_check) > 300 then
